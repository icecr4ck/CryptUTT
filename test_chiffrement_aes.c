...
void invMixColumns(char *stateIn, char *stateOut)
{
    for(int j=0; j<4; j++)
    {   
        stateOut[0+j*4] = MulE[stateIn[0+j*4]] ^ MulB[stateIn[1+j*4]] ^ MulD[stateIn[2+j*4]] ^ Mul9[stateIn[3+j*4]];
        stateOut[1+j*4] = MulE[stateIn[1+j*4]] ^ MulB[stateIn[2+j*4]] ^ MulD[stateIn[3+j*4]] ^ Mul9[stateIn[0+j*4]];
        stateOut[2+j*4] = MulE[stateIn[2+j*4]] ^ MulB[stateIn[3+j*4]] ^ MulD[stateIn[0+j*4]] ^ Mul9[stateIn[1+j*4]];
        stateOut[3+j*4] = MulE[stateIn[3+j*4]] ^ MulB[stateIn[0+j*4]] ^ MulD[stateIn[1+j*4]] ^ Mul9[stateIn[2+j*4]];
    }
}
...
{
    char cleCipher[16] = "7o3jk4rhf32k1k5p";
    char cleDecipher[16] = "7o3jk4rhf32k1k5p";
    char clair[100]="Bonjour Hugo ! Peux-tu déchiffrer ce message ? Putain de bordel de merde !!!!!";
    int tailleClair = strlen(clair);
    char stateIn[16] = "";
    char *stateOut = malloc(16*sizeof(char));
    char *cipher = malloc(100*sizeof(char));
    char *decipher = malloc(100*sizeof(char));
    int n=0, m=0;
    printf("%d\n", tailleClair);
    if (tailleClair > 16)
    {
        ...
        for (int i=0; i<n; i++)
        {
            for (int j=0; j<16; j++)
            {
                stateIn[j] = clair[16*i+j];
            }
            roundChiffrementAES(stateIn, stateOut, cleCipher, 10);
            for (int k=0; k<16; k++)
            {
                cipher[16*i+k] = stateOut[k];
            }
        }
        if (tailleClair%16 != 0)
        {
            ...
            for (int j=0; j<m; j++)
            {
                stateIn[j] = clair[16*n+j];
            }
            roundChiffrementAES(stateIn, stateOut, cleCipher, 10);
            for (int k=0; k<16; k++)
            {
                cipher[16*n+k] = stateOut[k];
            }
        }
    }
    else
    {
        roundChiffrementAES(clair, stateOut, cleCipher, 10);
        for (int k=0; k<strlen(stateOut); k++)
        {
            cipher[k] = stateOut[k];
        }
    }
    int tailleCipher = strlen(cipher);
    //printf("%d\n", tailleCipher);
    // Déchiffrement
    if (tailleCipher > 16)
    {
        ...
        for (int i=0; i<n; i++)
        {
            for (int j=0; j<16; j++)
            {
                stateIn[j] = cipher[16*i+j];
            }
            roundDechiffrementAES(stateIn, stateOut, cleDecipher, 10);
            for (int k=0; k<16; k++)
            {
                printf("%c", stateOut[k]);
            }
        }
        if (tailleCipher%16 != 0)
        {
            m = tailleCipher%16;
            for (int j=0; j<m; j++)
            {
                stateIn[j] = cipher[16*n+j];
            }
            roundDechiffrementAES(stateIn, stateOut, cleDecipher, 10);
            for (int k=0; k<strlen(stateOut); k++)
            {
    ...
    {
        roundDechiffrementAES(cipher, stateOut, cleDecipher, 10);
        for (int k=0; k<strlen(stateOut); k++)
        {
            printf("%c", stateOut[k]);
        }
