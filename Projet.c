...
void invMixColumns(unsigned char *stateIn, unsigned char *stateOut)
{
    ...
    for (int i = 0; i < 4; ++i) //Pour chaque ligne
    {
        int temp=0;
        // Mélange de la première colonne
        temp = (0x0e*stateIn[0+i*4]) ^ (0x0b*stateIn[1+i*4]) ^ (0x0d*stateIn[2+i*4]) ^ (0x09*stateIn[3+i*4]);
        temp = moduloMixColumns(temp);
        stateOut[0+i*4]=(char)temp;
        // Mélange de la deuxième colonne
        temp = (0x09*stateIn[0+i*4]) ^ (0x0e*stateIn[1+i*4]) ^ (0x0b*stateIn[2+i*4]) ^ (0x0d*stateIn[3+i*4]);
        temp = moduloMixColumns(temp);
        stateOut[1+i*4]=(char)temp;
        // Mélange de la troisième colonne
        temp = (0x0d*stateIn[0+i*4]) ^ (0x09*stateIn[1+i*4]) ^ (0x0e*stateIn[2+i*4]) ^ (0x0b*stateIn[3+i*4]);
        temp = moduloMixColumns(temp);
        stateOut[2+i*4]=(char)temp;
        // Mélange de la dernière colonne
        temp = (0x0b*stateIn[0+i*4]) ^ (0x0d*stateIn[1+i*4]) ^ (0x09*stateIn[2+i*4]) ^ (0x0e*stateIn[3+i*4]);
        temp = moduloMixColumns(temp);
        stateOut[3+i*4]=(char)temp;
    }
}
...
void invKeyExpansion(char *cle)
{
    ...
    for (int i = 0;i < 4; i++){
        cle[i] = sbytesInv[(unsigned char)cle[i]];
    }
}
...
void choixTailleCleChiffrementAES()
{
    int choixMenu;
...
void dechiffrerAES(int tailleCle, int Nr)
{
    unsigned char stateIn[16];
    unsigned char stateOut[16];
    int tailleCleByte = tailleCle/8;
    ...
    else
    {
        while (!feof(cipher))
        {
            ...
            for (int i=0; i<16; i++)
            {
                fputc(stateOut[i], decipher);
            }
        }
        fclose(cipher);
        fclose(decipher);
        printf("\nFélicitations, votre fichier a bien été déchiffré !\n\n");
    }
}

void choixTailleCleDechiffrementAES()
{
    int choixMenu;
    printf("Vous avez choisi le déchiffrement AES !\n\n");
    printf("--> Sélectionner la taille de clé <--\n");
    printf("1.128 bits\n");
    printf("2.192 bits\n");
    printf("3.256 bits\n");
    ...
    {
    case 1:
        dechiffrerAES(128,10);
        break;
    case 2:
        dechiffrerAES(192,12);
        break;
    case 3:
        dechiffrerAES(256,14);
        break;
    default:
        printf("Erreur, taille de clé incorrecte !\n\n");
        break;
...
int main(int argc, char *argv[])
{
    ...
    printf("2.Déchiffrement AES\n");
    printf("3.Chiffrement El-Gamal\n");
    printf("4.Signature El-Gamal\n");
    printf("Votre choix: ");
    scanf("%d", &choixMenu);
    switch(choixMenu)
    {
        case 1:
            choixTailleCleChiffrementAES();
            break;
        case 2:
            choixTailleCleDechiffrementAES();
            break;
        case 3:
            chiffrementElGamal();
            break;
